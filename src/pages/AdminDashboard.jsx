...
const bookings = [
  { guest: 'Eleanor James', room: 'Suite 402', date: 'Oct 12 - 15', status: 'Confirmada', amount: '$1,240' },
  { guest: 'Marcus Holloway', room: 'Room 108', date: 'Oct 14 - 16', status: 'Pendiente', amount: '$580' },
  { guest: 'Sarah Miller', room: 'Penthouse 01', date: 'Oct 15 - 20', status: 'Confirmada', amount: '$4,500' },
];

const tasks = [
  { title: 'Limpieza programada', room: 'Penthouse Suite 01', detail: '2:00 PM' },
  { title: 'Lista para check-in', room: 'Deluxe Double 204', detail: 'Housekeeping finalizado' },
  { title: 'Mantenimiento requerido', room: 'Standard King 312', detail: 'Revision de aire acondicionado' },
];
          ...
          <article className="admin-table-card">
            ...
            <div className="admin-booking-table">
              {bookings.map((booking) => (
                <div className="admin-booking-row" key={`${booking.guest}-${booking.room}`}>
                  <span>{booking.guest}</span>
                  <span>{booking.room}</span>
                  <span>{booking.date}</span>
                  <span className={booking.status === 'Confirmada' ? 'confirmed' : 'pending'}>
                    {booking.status}
                  </span>
          ...
          <article className="admin-revenue-card">
            ...
            <div className="admin-bars" aria-hidden="true">
              {[42, 58, 86, 72, 66, 82, 54].map((height, index) => (
                <span className={index === 2 || index === 5 ? 'highlight' : ''} key={height} style={{ height: `${height}%` }} />
              ))}
            </div>
